文档地址：http://w2.test.idcsmart.com/doc?name=app\\home\\controller\\RegisterController::resetPhoneSend

## 接口：短信密码重置发送验证码

接口地址：/reset\_phone\_send POST

短信密码重置发送验证码 -- wyh

  

**接口说明:阿里大鱼发送短信验证码或PHPMailer发送邮件验证码--密码重置(username)**

  

-   [接口信息](#info)
-   [在线测试](#test)

### 接口参数

参数名字

类型

是否必须

默认值

其他

说明

phone\_code

字符串

必填

1

\-

区号

phone

字符串

必填

1

\-

手机号

mk

字符串

必填

\-

\-

common\_list接口返回的msfntk作为cookie写入,并在发送短信时作为token传入

  

### 返回结果

`{     "status":200/201/203/204/400/401/406,     "msg":提示信息,     "data":{     }   }`

  

### 接口参数

接口地址

发送测试

提交方式

GET POST PUT DELETE

Cookie

   

增加参数

phone\_code

phone

mk

### 返回结果

自定义解析数据 缩进量: 1 2 3 4 5 6  引号 [全选](javascript:void(0);) [展开](javascript:void(0);) [叠起](javascript:void(0);) [2级](javascript:void(0);) [3级](javascript:void(0);) [4级](javascript:void(0);) [5级](javascript:void(0);) [6级](javascript:void(0);) [7级](javascript:void(0);) [8级](javascript:void(0);)

  
