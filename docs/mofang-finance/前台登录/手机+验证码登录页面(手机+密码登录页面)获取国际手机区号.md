文档地址：http://w2.test.idcsmart.com/doc?name=app\\home\\controller\\LoginController::mobileLoginVerifyPage

## 接口：手机+验证码登录页面(手机+密码登录页面)获取国际手机区号

接口地址：/mobile\_login\_page GET

手机+验证码登录页面(手机+密码登录页面)获取国际手机区号 -- wyh
