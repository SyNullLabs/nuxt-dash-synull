文档地址：http://w2.test.idcsmart.com/doc?name=app\\res\\controller\\ResourceHomeController::testResourceLink

## 接口：测试链接资源池(同时 资源池尝试连接供应商 握手)

接口地址：admin/addons?\_plugin=idcsmart\_res\_supplier&\_controller=AdminIndex&\_action=testResourceLink post

测试链接资源池(同时 资源池尝试连接供应商 握手) -- wyh

  

**接口说明:测试链接资源池(同时 资源池尝试连接供应商 握手)**
