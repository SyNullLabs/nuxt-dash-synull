文档地址：http://w2.test.idcsmart.com/doc?name=app\\res\\controller\\ResourceHomeController::postResourceShopLogoDelete

## 接口：删除店铺标志

接口地址：/resource/resourceshoplogodelete(admin/addons?\_plugin=idcsmart\_res\_supplier&\_controller=AdminIndex&\_action=postResourceShopLogoDelete) POST
...
店铺ID（编辑时传,页面返回有ID就传,没有ID就不调此接口）
