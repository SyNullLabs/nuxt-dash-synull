文档地址：http://w2.test.idcsmart.com/doc?name=app\\admin\\controller\\ClientsServicesController::refund

## 接口：退款

接口地址：/admin/clients\_services/refund POST

退款 -- wyh

  

**接口说明: 退款**

  

-   [接口信息](#info)
-   [在线测试](#test)

### 接口参数

参数名字

类型

是否必须

默认值

其他

说明

hid

整型

必填

\-

\-

主机id(产品ID)

refund\_method

字符串

必填

\-

\-

退款方案:'day'=>'按天计算','full'=>'全额退款','custom'=>'自定义'

refund\_type

字符串

必填

\-

\-

退款类型:'addascredit'=>'退款至余额','only'=>'仅标记退款'

amount

字符串

非必填

\-

\-

退款金额(仅自定义传此参数)

  

### 返回结果

`{     "status":200/201/203/204/400/401/406,     "msg":提示信息,     "data":{     }   }`

  

### 接口参数

接口地址

发送测试

提交方式

GET POST PUT DELETE

Cookie

   

增加参数

hid

refund\_method

refund\_type

amount

### 返回结果

自定义解析数据 缩进量: 1 2 3 4 5 6  引号 [全选](javascript:void(0);) [展开](javascript:void(0);) [叠起](javascript:void(0);) [2级](javascript:void(0);) [3级](javascript:void(0);) [4级](javascript:void(0);) [5级](javascript:void(0);) [6级](javascript:void(0);) [7级](javascript:void(0);) [8级](javascript:void(0);)

  
