文档地址：http://w2.test.idcsmart.com/doc?name=app\\res\\controller\\ResourceShopController::getEvaluation

## 接口：商品评论

接口地址：/resource/shop/evaluation get

商品评论 -- xj

  

**接口说明:商品评论**

  

-   [接口信息](#info)
-   [在线测试](#test)

### 接口参数

参数名字

类型

是否必须

默认值

其他

说明

page

整型

非必填

\-

\-

页码

limit

整型

非必填

\-

\-

长度

page

整型

非必填

1

\-

页数

limit

整型

非必填

10

\-

每页条数

order

字符串

非必填

create\_time

\-

排序(create\_time,score)

sort

字符串

非必填

\-

\-

排序规则(asc/desc)

type

字符串

非必填

\-

\-

类型great好评bad差评middle中评with\_img有图

id

字符串

必填

10

\-

商品ID

  

### 返回结果

`{     "status":200/201/203/204/400/401/406,     "msg":提示信息,     "data":{     }   }`

  

### 接口参数

接口地址

发送测试

提交方式

GET POST PUT DELETE

Cookie

   

增加参数

page

limit

page

limit

order

sort

type

id

### 返回结果

自定义解析数据 缩进量: 1 2 3 4 5 6  引号 [全选](javascript:void(0);) [展开](javascript:void(0);) [叠起](javascript:void(0);) [2级](javascript:void(0);) [3级](javascript:void(0);) [4级](javascript:void(0);) [5级](javascript:void(0);) [6级](javascript:void(0);) [7级](javascript:void(0);) [8级](javascript:void(0);)

  
